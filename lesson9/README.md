...
```bash
mkdir -p archived 
mv logs/old/* archived/ 2>/dev/null || true
```
...
```bash
du -sb logs | awk '{print $1}' > logs_size.txt
```
...
```bash
find logs -maxdepth 1 -type f -printf "%s %p\n" | sort -nr | head -n1 | awk '{print $2}' > biglog.txt
```
...
```bash
zip project_backup.zip $(find config -name "*.conf") $(find logs -name "*.log") errors.txt 2>/dev/null
```
...
```bash
for f in config/*.conf; do echo "$(basename "$f") $(wc -l < "$f")"; done > conf_stats.txt
```
